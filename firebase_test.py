...
def send_notifications(notifications):
    ...
    db.collection('notifications').document(
        str(uuid4())).create(notifications)
    end = time.time()
    spend_time = timedelta(seconds=end - start)
    return spend_time
...
def get_latest_metrics():
    ...
    try:
        db = firestore.client()
        ordered_metrics = db.collection('metrics').order_by(
...
def get_latest_long_or_short():
    ...
    if (notifications):
        latest_notification = list(
            map(lambda notification: notification.to_dict(), notifications))
